...
import GatePassEditDialog from '@/components/GatePassEditDialog';
...
const AdminGateDataPage: React.FC = () => {
  ...
  const [editGatePass, setEditGatePass] = useState<any>(null);
      ...
      if (gateData) {
        ...
        const [productRes, passRes, deptRes] = await Promise.all([
          ...
          supabase
            ...
            .eq('office_id', gateData.office_id)
            .order('created_at', { ascending: false }),
          supabase
            .from('departments')
            .select('id, name')
  ...
  const handleExportProducts = () => {
    ...
    exportToExcel(filteredProducts, columns, `gate-entries`);
  ...
  const handleExportPasses = () => {
    ...
    exportToExcel(filteredPasses, columns, `gate-passes`);
    toast.success('Exported successfully');
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-lg sm:text-xl font-bold truncate">{gate.name}</h1>
            <div className="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground flex-wrap">
              <Building2 className="h-3.5 w-3.5 flex-shrink-0" />
              <span className="truncate">{gate.offices?.name}</span>
              <span className="mx-1">•</span>
              <code className="text-[10px] sm:text-xs bg-secondary px-1.5 py-0.5 rounded">{gate.gate_code}</code>
            </div>
          </div>
        </div>
      ...
      <GateEntryPrintDialog
        ...
        products={products}
        gateName={gate.name}
        officeName={gate.offices?.name || 'Office'}
      />
      ...
      {editGatePass && (
        <GatePassEditDialog
          open={!!editGatePass}
          onOpenChange={(open) => !open && setEditGatePass(null)}
...
export default AdminGateDataPage;
